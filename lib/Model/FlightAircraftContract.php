...
/**
 * FlightAircraftContract Class Doc Comment
 *
 * @category Class
 ...
 */
class FlightAircraftContract implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'FlightAircraftContract';
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('mode_s', $data ?? [], null);
    ...
    */
     ...
     * Sets reg
     *
     * @param string|null $reg Tail-number of the aircraft
     *
     * @return self
     */
    ...
    {
        if (is_null($reg)) {
            array_push($this->openAPINullablesSetToNull, 'reg');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('reg', $nullablesSetToNull);
            if ($index !== FALSE) {
    ...
    {
        return $this->container['mode_s'];
    }

    /**
     * Sets mode_s
     *
     * @param string|null $mode_s ICAO 24 bit Mode-S hexadecimal transponder address
     *
     * @return self
     */
    public function setModeS($mode_s)
    {
        if (is_null($mode_s)) {
            array_push($this->openAPINullablesSetToNull, 'mode_s');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('mode_s', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        }
        $this->container['mode_s'] = $mode_s;
     ...
     * Sets model
     *
     * @param string|null $model Aircraft name and model
     *
     * @return self
     */
     ...
     * Sets image
     *
     * @param \OpenAPI\Client\Model\ResourceContract|null $image Aircraft photo. Either actual (possible if aircraft registration is provided) or approximate based on airline name and model combination.  Image data with medium-sized direct image URL and licence approved for commercial use is returned.
     *
     * @return self
     */
