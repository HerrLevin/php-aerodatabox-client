...
/**
 * DelayBracketContract Class Doc Comment
 *
 * @category Class
 ...
 */
class DelayBracketContract implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'DelayBracketContract';
    ...
    protected static $openAPIFormats = [
        'delayed_from' => 'date-span',
        'delayed_to' => 'date-span',
        'num' => 'int32',
        'percentage' => 'float'
    ];
    ...
    protected static $setters = [
        'delayed_from' => 'setDelayedFrom',
        'delayed_to' => 'setDelayedTo',
        'num' => 'setNum',
    ...
    protected static $getters = [
        'delayed_from' => 'getDelayedFrom',
        'delayed_to' => 'getDelayedTo',
        'num' => 'getNum',
    ...
    {
        $this->setIfExists('delayed_from', $data ?? [], null);
        $this->setIfExists('delayed_to', $data ?? [], null);
    ...
    */
     ...
     * @return string|null
     */
    public function getDelayedFrom()
    {
        return $this->container['delayed_from'];
    }

    /**
     * Sets delayed_from
     *
     * @param string|null $delayed_from The beginning of the delay range (format: [-]hh:mm:ss).  E.g. in the expresssion \"delayed from 30 to 60 minutes\" stands for \"from 30 minutes\".  Value can be negative (therefore, means early occurence).    Must be preset if \"DelayedTo\" is not specified.
     *
     * @return self
     */
    public function setDelayedFrom($delayed_from)
    {
        if (is_null($delayed_from)) {
            array_push($this->openAPINullablesSetToNull, 'delayed_from');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('delayed_from', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        }
        $this->container['delayed_from'] = $delayed_from;
    ...
    {
        return $this->container['delayed_to'];
    }

    /**
     * Sets delayed_to
     *
     * @param string|null $delayed_to The end of the delay range (format: [-]hh:mm:ss).  E.g. in the expresssion \"delayed from 30 to 60 minutes\" stands for \"to 60 minutes\".  Value can be negative (therefore, means early occurence).    Must be preset if \"DelayedFrom\" is not specified.
     *
     * @return self
     */
    public function setDelayedTo($delayed_to)
    {
        if (is_null($delayed_to)) {
            array_push($this->openAPINullablesSetToNull, 'delayed_to');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('delayed_to', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        }
        $this->container['delayed_to'] = $delayed_to;
     ...
     * Sets num
     *
     * @param int $num Count of records subject to a specified delay range
     *
     * @return self
     */
     ...
     * Sets percentage
     *
     * @param float|null $percentage Percentage of records subject to a specified delay range.    Only available when this delay bracket is provided as a part  of a statistical information of a bigger context and the \"total\"  count of records is known.
     *
     * @return self
     */
